...
function ScreenplayInner() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND}/api/stage/generate`, {
        ...
        body: JSON.stringify({ project_id: pid, stage: 'screenplay', tweak, engine })
      })
      ...
      setErr(e.message || 'Failed to generate screenplay beats')
    } finally {
      setLoading(false)
    }
  }
  ...
  const choose = async (id: string) => {
    try {
      ...
      const res = await fetch(`${BACKEND}/api/stage/choose`, {
        method:'POST',
        headers:{ 'Content-Type':'application/json', Authorization:`Bearer ${token}` },
        body: JSON.stringify({ project_id: pid, stage: 'screenplay', chosen_id: id, edits: edits[id] || '' })
      })
      if (!res.ok) throw new Error(await res.text())
      router.push(`/flows/new/script?pid=${encodeURIComponent(pid)}&engine=${engine}&lang=${lang}`)
    } catch (e:any) {
      setErr(e.message || 'Failed to choose beats')
    }
  }
  ...
  return (
    <main className="max-w-5xl mx-auto px-6 py-8 space-y-6">
      <h1 className="text-2xl font-semibold">Step 4: Screenplay — Scene Beats (3 options)</h1>
      ...
      <div className="rounded-xl bg-neutral-900/50 border border-neutral-800 p-4 space-y-3">
        ...
        <button onClick={generate} disabled={loading}
          ...
          {loading ? 'Generating…' : 'Regenerate 3 beat options'}
        </button>
