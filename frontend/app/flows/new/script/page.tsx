...
export default function Page() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND}/api/stage/generate`, {
        ...
        body: JSON.stringify({ project_id: pid, stage: 'script', tweak, engine })
      })
      ...
      setErr(e.message || 'Failed to generate script pages')
    } finally {
      setLoading(false)
    }
  }
  ...
  const choose = async (id: string) => {
    try {
      ...
      const res = await fetch(`${BACKEND}/api/stage/choose`, {
        method:'POST',
        headers:{ 'Content-Type':'application/json', Authorization:`Bearer ${token}` },
        body: JSON.stringify({ project_id: pid, stage: 'script', chosen_id: id, edits: edits[id] || '' })
      })
      if (!res.ok) throw new Error(await res.text())
      const nextQs = new URLSearchParams({ pid, engine, lang })
      router.push(`/flows/new/dialogue?${nextQs.toString()}`)
    } catch (e:any) {
      setErr(e.message || 'Failed to choose script pages')
    }
  }
  ...
  return (
    <main className="max-w-5xl mx-auto px-6 py-8 space-y-6">
      <h1 className="text-2xl font-semibold">Step 5: Script Pages (3 options)</h1>

      <div className="grid md-grid-cols-3 md:grid-cols-3 gap-4">
        {(cands||[]).map(c => (
      ...
      <div className="rounded-xl bg-neutral-900/50 border border-neutral-800 p-4 space-y-3">
        ...
        <button onClick={generate} disabled={loading}
          ...
          {loading ? 'Generating…' : 'Regenerate 3 script options'}
        </button>
