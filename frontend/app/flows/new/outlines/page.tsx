...
export default function Page() {
  ...
  // First load: generate once pid+user are ready
  useEffect(() => {
    if (canGo) generate()
    ...
    <main className="max-w-5xl mx-auto px-6 py-8 space-y-6">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {(cands || []).map((c) => (
          <div
            key={c.id}
            className="rounded-xl bg-neutral-900/60 border border-neutral-800 p-4 flex flex-col gap-3"
          >
      ...
      <div className="rounded-xl bg-neutral-900/50 border border-neutral-800 p-4 space-y-3">
        <label className="block text-sm">
          Regenerate with a steering note (optional)
        </label>
        <input
